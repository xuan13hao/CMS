...
public class BaseDaoImp<T extends Serializable> implements BaseDao<T> {
	...
	{
		System.out.println("baeDaoImp:"+cm.getConnection());
	...
	{
		int result=0;
		ps=cm.getConnection().prepareStatement(sql);
	...
	public void delete(Class<T> clazz,Integer id) throws SQLException 
	{
		...
		if(null!=id&&id>0)
		{
			...
			this.updateBySql(sql,new Object[]{});
			log.info(sql);
		}else
		{
			log.error("删除数据不能没有ID");
			throw new SQLException("没有id");
		}
	}
...
/*	@Override
			...
			Object... params) throws SQLException 
	{
		...
		List<T> list=new ArrayList<T>();
		ps=cm.getConnection().prepareStatement(sql+" limit "+top);
		setParamsToPs(ps,params);
		ResultSet rs=ps.executeQuery();
		while(rs.next())
		{
	...
	{
		List<T> list=new ArrayList<T>();
		ps=cm.getConnection().prepareStatement(sql+" limit "+(pageNo-1)*pageSize+","+pageSize);
		setParamsToPs(ps,params);
		ResultSet rs=ps.executeQuery();
    	while(rs.next())
		{
